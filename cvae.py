...
class CVAE:
    ...
    def _nn_layer(self, inputs, n_hidden, initializer_type='He', add_bias=False, name=None, activation=None,
                  stddev=None):
    ...
    def reconstruction(self):
        ...
        epsilon = 1e-10
        recons = tf.reduce_sum(
            self.X * tf.log(epsilon + self.output) + (1 - self.X) * tf.log(epsilon + 1 - self.output), axis=1)
        recons = - recons
    ...
    def regularization(self):
        # KL-divergence
        regulars = 0.5 * tf.reduce_sum(
            -1 - tf.log(1e-8 + tf.square(self.sigma)) + tf.square(self.mu) + tf.square(self.sigma), axis=1)
        regulars = tf.reduce_mean(regulars)

        return regulars

    def build(self):
        # concat with Label Y (Condition on laten space)
        self.z_encoder_layer = tf.concat([self.X, self.Y], axis=1)

        # Encoder
        count = 0
        for n_hidden in self.n_hidden_list:
            variables_name = 'z_{}dim_encoder_{}'.format(self.z_dim, count)
            self.z_encoder_layer = self._nn_layer(self.z_encoder_layer, n_hidden=n_hidden, activation=tf.nn.elu,
                                                  name=variables_name)
            count += 1
        ...
        # concat with Label Y (Condition on output)
        self.z_decoder_layer = tf.concat([self.sampled_z, self.Y], axis=1)
        # Decoder
        count = 0
        for n_hidden in reversed(self.n_hidden_list):
            variables_name = 'z_{}dim_decoder_{}'.format(self.z_dim, count)
            self.z_decoder_layer = self._nn_layer(self.z_decoder_layer, n_hidden=n_hidden, activation=tf.nn.elu,
                                                  name=variables_name)
            count += 1

        X_size = self.X.get_shape().as_list()[-1]
        self.output = self._nn_layer(self.z_decoder_layer, n_hidden=X_size, activation=tf.nn.sigmoid,
                                     name="z_{}dim_reconstruction".format(self.z_dim))
